...
class Controller_moderator_list extends Controller{
    
    public function action_moderator_pagination() {
        ...
        if (isset($_GET['start'])) {
            if (preg_match("/^[1-9]\d*$/", $_GET['start'])) {
                ...
                $printab = $m->getModeratorsWithLimit($offset, $itemsPerPage);
                $pages = $m->getNbModerator()/25;
                if (is_float($pages)) {
                    $pages = ceil($pages);
                }
		if ($pages == 0) {
		$data = [
                ...
                if ($page > $pages) {
                    ...
                    $this->render("moderator_pagination", $data);
                }
            }
            else {
                $_GET['start'] = 1;
                header("Location: ?controller=moderator_list&action=moderator_pagination&start=" . $_GET['start']);
            }
    ...
    public function action_default(){
        $this->action_moderator_pagination();
    }
    ...
    public function action_moderator_information_admin() {
        ...
        if (isset($_GET['id']) && preg_match("/^[1-9][0-9]*$/", $_GET['id'])) {
            ...
            if ($data['infos']) {
                ...
                $this->render("moderator_information_admin", $data);
            }
            else {
    ...
    public function action_moderator_information_moderator() {
        ...
        if (isset($_GET['id']) && preg_match("/^[1-9][0-9]*$/", $_GET['id'])) {
            ...
            if ($data['infos']) {
                ...
                $this->render("moderator_information_moderator", $data);
            }
            else {
                $this->action_error("L'identifiant n'existe pas !");
            }
